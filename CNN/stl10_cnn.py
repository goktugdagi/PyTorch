"""
STL-10 Image Classification with a Custom CNN (PyTorch)

Features:
- Augmentation + normalization
- Custom lightweight CNN (3 conv blocks + GAP + classifier)
- Mixed precision (AMP)
- StepLR scheduler + EarlyStopping
...
ACC_PLOT = os.path.join(OUTPUT_DIR, "acc_curves.png")
...
# -------------------------------------------------
# Pre-computed mean and std for STL-10 (in RGB order)
STL_MEAN = [0.447412, 0.427196, 0.386978]
STL_STD = [0.26034, 0.256438, 0.271819]
...
# -------------------------------------------------
def load_stl10_datasets(data_root="./data"):
    """
    ...
    print(f"Image shape: {image.shape} -> [channels, height, weight]")
...
def create_dataloaders(trainval_aug, full_val_clean, test_dataset):
    ...
    NUM_WORKERS = 0 # Set > 0 if your environment support multiprocessing well
...
def eval_model(
        model: nn.Module,
        data_loader_or_dataset,   # DİKKAT: isim burada değişti
        loss_fn,
        device=device
):
    """
    ...
    """
    # Allow passing a raw Dataset instead of Dataloader
...
class CNN(nn.Module):
    ...
    """
    Hafif CNN:
    - Giriş: 96x96 RGB (3 kanal)
    - 3 konvolüsyon bloğu + MaxPool
    - Global (adaptive) average pooling
    - Tek tam bağlantılı (Linear) katman
    """
    ...
    def __init__(self, input_shape: int, 
        ...
        # Block 3 -> final convolution to deepen features
